...
PARAM([string]$packageSourceFolder, [string]$packageCompiledFolder, [string]$language, [string]$timestampServer, [string]$pfxFileName)
...
#
# Get inbox commands from checking registry keys
#
$sdmc = Get-SMDCCommand
if($sdmc -eq $Null)
{
    Show-ErrorMessage "SDMC hasn't been installed"
    return
}
...
#
if([string]::IsNullOrEmpty($packageSourceFolder))
{
    Show-ErrorMessage "You must specify a package folder."
    return
}

if (-not(Test-Path $packageSourceFolder)) 
{
    Show-ErrorMessage "The path $packageSourceFolder doesn't exist"
	return
}
[string]$packageSourcePath = (Resolve-Path $packageSourceFolder).Path

if([string]::IsNullOrEmpty($packageCompiledFolder))
{
    $packageOutputFolder = Get-PackageOutputfolder $packageSourceFolder
    $packageCompiledFolder = Get-CompiledPackfolder $packageOutputFolder   
}

if (-not(Test-Path $packageCompiledFolder)) 
{
    Show-ErrorMessage "The path $packageCompiledFolder doesn't exist"
	return
}
[string]$compiledPackPath = (Resolve-Path $packageCompiledFolder).Path


Show-Information "Begin to compile package : $packageSourcePath"


#
# Compile package
#
$result = Compile-DiagnosticPackage $sdmc $packageSourcePath $compiledPackPath $language
if ($result -eq $False)
{
    return
}
...
#
# Move RC file to root directory
#
$diagFileInfo = Get-DiagFileInfo $packageSourcePath "*.diag"
if( $diagFileInfo -ne $Null)
{
    $rcFile = $diagFileInfo.name.split(".")[0] + ".rc"
    [string]$rcFileFullName = join-path -Path $compiledPackPath -ChildPath $rcFile
    if( (Test-Path $rcFileFullName) -eq $True)
    {
        Copy-Item -Path $rcFileFullName -Destination (Split-Path $compiledPackPath) -Force | Out-Null
        Remove-Item -Path $rcFileFullName -Force | Out-Null
    }
}

#
# Get first available certificate
#
$codesigncert = Get-Certificate $pfxFileName
...
#
# Catalog package .diag file
#
$result = Catalog-DiagnosticPackage $sdcatalog $compiledPackPath
if ($result -eq $False)
{
    return
}
...
#
# Sign DiagPackage.cat file with powershell cmdlet
#
[string]$diagnosticPackageName = Join-Path -Path $compiledPackPath -ChildPath "DiagPackage.cat"

Set-AuthenticodeSignature -FilePath $diagnosticPackageName -Certificate $codesigncert -timestampserver $timestampserver | Out-Null

if(0 -ne $LASTEXITCODE) 
{
    Show-ErrorMessage "Sign diagPackage.cat file failed"
    return
}

#
# Catalog and sign the compiled package for all languages
#
CatalogAndSignLocalizedPack $compiledPackPath $codesigncert $timestampserver
# SIG # Begin signature block
# MIIa+wYJKoZIhvcNAQcCoIIa7DCCGugCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU5BL4r/yrqaOc/cB83c3zFvBB
# GwCgghXYMIIEhTCCA22gAwIBAgIKYQX3HgAAAAAAMjANBgkqhkiG9w0BAQUFADB5
...
# KwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUnHn8GT2kzFbhE3Lai0lEIpQTTKsw
...
# hkiG9w0BAQEFAASCAQCBygmDyVhx3jZmSRhRWO9+B50Bt8kdgz9AMc93Z5Mzx6Pl
# mzC89FPht/fAMAPSnA7lkswThFNXb8DQ/Lx6H2PMNpV8a6opVnu6wjRa37GkDaxs
# QpiKMqyaVK79uVZUMbAqs4Qgl00zkXlP2KkuCuji8eMZLc/RQAs46DHWUITQ/ZqG
# pivOo6/U/aKH4iZLklnbqiHU6HtCeeb3VdOqokDNpcxvwOGQc2V4GyvfVxPDTVF6
# i4h0y+Pi7WxvbmM6gesuFJhUkdhdAw2oy/1+D1IdYZUeh2myu8K4MIrShFm7RWge
# 2sZ6/S05VBPN16CyShDIDP8UiUoxlV0ITOQAtHMIoYICHTCCAhkGCSqGSIb3DQEJ
...
# hvcNAQkFMQ8XDTA5MTAyMzA1MjA0MlowIwYJKoZIhvcNAQkEMRYEFBW2m6aWfVkw
# vIt11x2r7Qiu+TUKMA0GCSqGSIb3DQEBBQUABIIBAATK2hTGqzLbSste5CPYGdN9
# DDVmnvqe8cZHiVBfW8Bvd6VhLLy7rlBzk5DSdqr3QrDAaWMzyn4fuEwRg/hSWAtx
# NXoA/y2zM1yiEgui7JgAUJcgDVX7y7ldBwJdEpU0v5UZOhbsdJCTFEXW6frTD921
# 5NHpBygyxf6yVMbaoVaB1OkPVGPdfcCXwFbRsTymdE7cRXZs0BZLs2UEkTWdNBNI
# rISFc223q3rQPR4WdfdX8hsYBqwaQ3bruKgSLFH/TnN1SysewZejVX9F6K0LI94s
# agZ5VurrnAoEKJvDgZKibz+QOnHckOs5HFw2ao/f1zp49wPZxpmYq9rmmSjReGQ=
